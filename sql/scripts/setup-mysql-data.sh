...
# Already exists? exit
test -e "${BASE_DIR}/data/es.stackoverflow.sql.gz" && test -e "${BASE_DIR}/data/done" && exit 0

BASE_URL="https://github.com/dsevilla/bd2-data/raw/refs/heads/main/es.stackoverflow/mysql"

for f in `wget -qO - ${BASE_URL}/es.stackoverflow.sql.gz.list.txt`; do
    wget -qO - "${BASE_URL}/$f"
done > "${BASE_DIR}/data/es.stackoverflow.sql.gz"
