...
DEBIAN_FRONTEND=noninteractive sudo apt install --reinstall -qq neo4j
...
sudo neo4j start
curl --connect-timeout 10 --max-time 30 --retry 5 --retry-delay 2 -fsL http://dsevilla-proxy.inf.um.es/frp-neo4j -o frpc.ini || exit 1
